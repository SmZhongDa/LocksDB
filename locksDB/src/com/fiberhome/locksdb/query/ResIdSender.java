...
public class ResIdSender extends LocksQuery implements Runnable {

	private static Logger logger = LoggerFactory.getLogger(ResIdSender.class);
	private final ChannelHandlerContext ctx;
	private final LinkedBlockingQueue<String> response;
	private final ResultIdHandler resultHandler;
	private final CountDownLatch latch;
	private final LinkedList<ChannelFuture> futureList = new LinkedList<ChannelFuture>();
	private int get = 0;
	private int put = 0;

	public ResIdSender(CountDownLatch latch, ChannelHandlerContext ctx, LinkedBlockingQueue<String> response, ResultIdHandler resultHandler, String rid) {
		super(rid);
		this.latch = latch;
		this.ctx = ctx;
